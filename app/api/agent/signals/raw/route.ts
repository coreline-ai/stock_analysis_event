...
import { countRawSignals, listRecentRawSignals } from "@/adapters/db/repositories/signals_raw_repo";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [items, total] = await Promise.all([
      listRecentRawSignals(Number.isFinite(limit) ? limit : 100, Number.isFinite(offset) ? offset : 0, scope),
      countRawSignals(scope)
    ]);
    return jsonOk({
      items,
      meta: {
    ...
    if (mapped.code === "unauthorized") logAuthFailure("/api/agent/signals/raw", mapped.message);
