...
const TOKEN = process.env.API_TOKEN ?? process.env.DEEPSTOCK_API_TOKEN ?? "dev-token";
...
async function main() {
  ...
  try {
    const picked = await pickTargetSignal();
    if (!picked) {
      console.log("[feature] scored signal not found (KR/US). skipping feature assertions.");
      return;
    }
    const targetScope = picked.scope;
  ...
  if (targetScope === "KR") {
    ...
    if (dropdownVisible) {
      ...
      console.log("[feature] KR autocomplete check passed (dropdown)");
    } else {
      ...
      if (rowCount > 0) {
        ...
        assert.ok(text.trim().length > 0, "KR 이름 필터 반영 실패");
        console.log("[feature] KR autocomplete check passed (table fallback)");
      } else {
        console.log("[feature] KR autocomplete check skipped (no table rows after filter)");
      }
    }
  } else {
    console.log("[feature] KR autocomplete check skipped (US-only dataset)");
  }
  ...
  for (const vp of [
    ...
    await vpPage.goto(
      `${BASE_URL}/dashboard/signals?tab=scored&scope=${targetScope}&scoredId=${encodeURIComponent(target.id ?? "")}`,
      {
