...
if [[ "${USE_LAUNCHCTL:-false}" == "true" ]] && [[ "$(uname -s)" == "Darwin" ]] && command -v launchctl >/dev/null 2>&1; then
  uid="$(id -u)"
  if launchctl print "gui/${uid}/${AGENT_LABEL}" >/tmp/deepstock-launchctl-status.txt 2>/dev/null; then
    launch_pid="$(grep -m 1 'pid = ' /tmp/deepstock-launchctl-status.txt | sed -E 's/.*pid = ([0-9]+).*/\1/' || true)"
